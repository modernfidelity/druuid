...
class DrUUID {
  ...
  public static function v3($namespace, $name) {
    ...
    for ( $i = 0; $i < drupal_strlen($nhex); $i+=2 ) {
    ...
    return sprintf('%08s-%04s-%04x-%04x-%12s',
                    ...
                    drupal_substr($hash, 0, 8),
                    // 16 bits for "time_mid"
                    drupal_substr($hash, 8, 4),
                    // 16 bits for "time_hi_and_version",
                    // four most significant bits holds version number 3
                    (hexdec(drupal_substr($hash, 12, 4)) & 0x0fff) | 0x3000,
                    // 16 bits, 8 bits for "clk_seq_hi_res",
                    // 8 bits for "clk_seq_low",
                    // two most significant bits holds zero and one for variant DCE1.1
                    (hexdec(drupal_substr($hash, 16, 4)) & 0x3fff) | 0x8000,
                    // 48 bits for "node"
                    drupal_substr($hash, 20, 12)
    );
  }
  ...
  public static function v5($namespace, $name) {
    ...
    return sprintf('%08s-%04s-%04x-%04x-%12s',
                    ...
                    (hexdec(drupal_substr($hash, 12, 4)) & 0x0fff) | 0x5000,
                    // 16 bits, 8 bits for "clk_seq_hi_res",
                    // 8 bits for "clk_seq_low",
                    // two most significant bits holds zero and one for variant DCE1.1
                    (hexdec(drupal_substr($hash, 16, 4)) & 0x3fff) | 0x8000,
